...
function networkUp () {
  ...
  docker exec cli scripts/script.sh $CHANNEL_NAME12 $CHANNEL_NAME34 $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
...
function upgradeNetwork () {
  ...
  docker exec cli scripts/upgrade_to_v11.sh $CHANNEL_NAME12 $CHANNEL_NAME34 $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "### Generating channel 12 configuration transaction 'channel12.tx' ###"
  echo "#################################################################"
  set -x
  configtxgen -profile 12OrgsChannel -outputCreateChannelTx ./channel-artifacts/channel12.tx -channelID $CHANNEL_NAME12
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel 12 configuration transaction..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "### Generating channel 34 configuration transaction 'channel34.tx' ###"
  echo "#################################################################"
  set -x
  configtxgen -profile 34OrgsChannel -outputCreateChannelTx ./channel-artifacts/channel34.tx -channelID $CHANNEL_NAME34
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel 34 configuration transaction..."
    exit 1
  fi
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile 12OrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME12 -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile 12OrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME12 -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile 34OrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/Org3MSPanchors.tx -channelID $CHANNEL_NAME34 -asOrg Org3MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile 34OrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/Org4MSPanchors.tx -channelID $CHANNEL_NAME34 -asOrg Org4MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
# Announce what was requested
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
        echo
        echo "${EXPMODE} with channel '${CHANNEL_NAME12} | ${CHANNEL_NAME34}' and CLI timeout of '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${IF_COUCHDB}'"
  else
        echo "${EXPMODE} with channel '${CHANNEL_NAME12} | ${CHANNEL_NAME34}' and CLI timeout of '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds"
  fi
